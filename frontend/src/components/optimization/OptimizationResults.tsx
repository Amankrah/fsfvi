...
import { ComponentOptimizationResult } from '@/lib/api';
...
export const OptimizationResults: React.FC<OptimizationResultsProps> = ({ result }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-900">
                  {formatPercent(result.relative_improvement_percent)}
                </p>
                <p className="text-xs text-blue-700 mt-1">
                  {formatNumber(result.optimal_fsfvi)} ← {formatNumber(result.original_fsfvi)}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-900">
                  {formatPercent(result.efficiency_gain_percent)}
                </p>
        ...
        <Card className="border-0 shadow-lg bg-gradient-to-br from-purple-50 to-violet-50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-purple-600">Reallocation</p>
                <p className="text-3xl font-bold text-purple-900">
                  {formatCurrency(result.total_reallocation_amount)}
                </p>
                <p className="text-xs text-purple-700 mt-1">
                  {formatPercent(result.reallocation_intensity_percent)} intensity
                </p>
              </div>
      ...
      {result.component_analysis && result.component_analysis.components && result.component_analysis.components.length > 0 && (
        <Card className="border-0 shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <BarChart3 className="w-5 h-5 text-blue-600" />
              <span>Component Allocation Changes</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="mb-4 p-4 bg-gray-50 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                <div className="text-center">
                  <p className="text-sm text-gray-600">Budget Increased</p>
                  <p className="text-2xl font-bold text-green-600">
                    {safeValue(result.component_analysis.summary?.components_increased)}
                  </p>
                  <p className="text-xs text-gray-500">components</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-gray-600">Budget Decreased</p>
                  <p className="text-2xl font-bold text-red-600">
                    {safeValue(result.component_analysis.summary?.components_decreased)}
                  </p>
                  <p className="text-xs text-gray-500">components</p>
                </div>
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {result.component_analysis.summary?.components_improved || 0}
                  </p>
                  <p className="text-xs text-gray-500">
                    ({result.component_analysis.summary?.components_sacrificed || 0} strategic tradeoffs)
                  </p>
                </div>
              </div>
              
              {result.component_analysis.summary?.optimization_explanation?.strategic_reallocation && (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {result.component_analysis.components.map((component, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-3 px-4">
                        <div className="font-medium text-gray-900">{component.component_name || 'Unknown'}</div>
                        <div className="text-xs text-gray-500">{component.component_type || 'Unknown'}</div>
                      </td>
            ...
            {result.component_analysis.recommendations && result.component_analysis.recommendations.length > 0 && (
              <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                ...
                <div className="space-y-2">
                  {result.component_analysis.recommendations.map((recommendation, index) => (
                ...
                <div className="mt-3 p-2 bg-green-100 rounded-md">
                  <p className="text-xs text-green-700">
                    💡 <strong>Generated by AI optimization:</strong> These recommendations are automatically generated based on the mathematical optimization analysis and highlight the most impactful budget reallocations.
                  </p>
                </div>
              </div>
            )}
